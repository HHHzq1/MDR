...
class MSDTrainer(BaseTrainer):
    ...
    def train(self, clip_model_dict=None, bert_model_dict=None):
        ...
        with tqdm(total=self.train_num_steps, postfix='loss:{0:<6.5f}', leave=False, dynamic_ncols=True,
            ...
            self.pbar = None
            self.logger.info("Get best dev performance at epoch {}, best dev f1 score is {}, best dev accuracy is {}".format(
                self.best_dev_epoch, self.best_dev_metric, self.best_dev_acc))
            # self.logger.info(
    ...
    def evaluate(self, epoch):
        ...
        with torch.no_grad():
            with tqdm(total=len(self.dev_data), leave=False, dynamic_ncols=True) as pbar:
                ...
                self.logger.info("Epoch {}/{}, best dev f1: {}, best f1 epoch: {}, best dev acc: {}, best acc epoch: {}, current dev f1 score: {}, acc: {}." \
                                 .format(epoch, self.args.num_epochs, self.best_dev_metric, self.best_dev_epoch,
                                         self.best_dev_acc, self.best_dev_acc_epoch, micro_f1, acc))
                
                if acc >= self.best_dev_acc:
                    self.logger.info("Get better accuracy at epoch {}".format(epoch))
                    self.best_dev_acc = acc
                    self.best_dev_acc_epoch = epoch

                if micro_f1 >= self.best_dev_metric:  # this epoch get best performance
                    self.logger.info("Get better f1 performance at epoch {}".format(epoch))
    ...
    def _step(self, batch, mode="train"):
        ...
        if len(batch_items) == 6:
            input_ids, input_mask, segment_ids, img_mask, labels, images = batch_items
